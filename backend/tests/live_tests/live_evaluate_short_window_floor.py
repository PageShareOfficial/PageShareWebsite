...
Writes: tests/docs/live_evaluate_short_window_floor.md
Run from backend/:
  $env:PYTHONPATH = "."
  python tests/docs/live_evaluate_short_window_floor.py
"""
...
OUT_PATH = DOCS_DIR / "live_evaluate_short_window_floor.md"
ASSET = "BTC"

def _load_trace_helpers():
    path = DOCS_DIR / "live_evaluate_zoom_trace.py"
    spec = importlib.util.spec_from_file_location("live_evaluate_zoom_trace", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Cannot load {path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module._fmt, module.trace_smart_search

_fmt, trace_smart_search = _load_trace_helpers()

...
export async function fetchStockData(ticker: string): Promise<StockData | null> {
  ...
  try {
    ...
    return {
      ...
      image: getStockLogoUrl(ticker, name),
    };
  } catch (error) {
...
async function fetchStockDataAlternative(ticker: string): Promise<StockData | null> {
  try {
    ...
    return {
      ...
      image: getStockLogoUrl(ticker, data.name || ticker),
    };
  } catch (error) {
...
async function fetchCryptoDataBySearch(ticker: string): Promise<StockData | null> {
  try {
    ...
    // Get coin image from coin detail
    const coinInfo = await getCryptoNameAndImage(coinId);
...
// Get crypto name and image from coin ID
async function getCryptoNameAndImage(coinId: string): Promise<{ name: string | null; image: string | null }> {
  try {
    ...
    console.error(`Error fetching crypto info for ${coinId}:`, error);
    return { name: null, image: null };
  }
}
...
export async function searchCrypto(query: string): Promise<SearchSuggestion[]> {
  ...
  try {
    ...
    return data.coins.slice(0, 10).map((coin: any) => ({
      ...
      image: coin.thumb || coin.small || coin.large || undefined,
    }));
  } catch (error) {
...
export async function searchTickers(query: string): Promise<SearchSuggestion[]> {
  ...
  const isLikelyStockTicker = normalizedQuery.length <= 5 && /^[A-Z]+$/.test(normalizedQuery);
  
  // Known crypto tickers that should be treated as crypto even if they match stock patterns
  const knownCryptoTickers = ['BTC', 'ETH', 'BNB', 'SOL', 'XRP', 'ADA', 'DOGE', 'DOT', 'MATIC', 'AVAX', 'LTC', 'LINK', 'UNI', 'ATOM', 'ETC', 'XLM', 'ALGO', 'VET', 'ICP'];
  const isKnownCrypto = knownCryptoTickers.includes(normalizedQuery);
  ...
  // If it's a likely stock ticker and not a known crypto, prioritize stock results
  // Filter out crypto results that match stock tickers to prevent false positives
  let filteredCryptoResults = cryptoResults;
  if (isLikelyStockTicker && !isKnownCrypto) {
    // Remove crypto results that match the exact ticker (likely false positive)
    filteredCryptoResults = cryptoResults.filter(
      (crypto) => crypto.ticker.toUpperCase() !== normalizedQuery
    );
  }

  // Combine results: prioritize stocks for likely stock tickers
  const combined = isLikelyStockTicker && !isKnownCrypto
    ? [...stockResults, ...filteredCryptoResults] // Stocks first
    : [...stockResults, ...cryptoResults]; // Normal order

  // Remove duplicates (same ticker) - keep the first occurrence
  ...
  return unique.slice(0, 10); // Limit to 10 total suggestions
}
